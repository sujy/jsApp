module.exports = function( grunt ) {
	...
	grunt.initConfig({
		...
		testswarm: {
			tests: "ajax attributes callbacks core styles data deferred dimensions effects event manipulation offset queue selector serialize support traversing Sizzle".split(" ")
		},
	...
	grunt.registerTask( "selector", "Build Sizzle-based selector module", function() {
		...
		/**
			...
			sizzle-jquery.scripts -> sizzle between "EXPOSE" blocks,
	...
	grunt.registerMultiTask(
		...
		function() {
			...
			//  *:*:-styles           all except styles and dependents (explicit > implicit)
			//  *:*:-styles:+effects  same (excludes effects because explicit include is trumped by explicit exclude of dependency)
	...
	// Change the map file to point back to jquery.scripts instead of jquery.pre-min.scripts.
	...
	grunt.registerMultiTask( "post-uglify", function() {
		...
		this.files.forEach(function( mapping ) {
			...
			grunt.file.write( mapFileName, grunt.file.read( mapping.dest )
				...
				.replace( /"dist\//g, "\"" )
				// Refer to the source jquery.scripts, not the temporary jquery.pre-min.scripts.
				.replace( /\.pre-min\./g, "." )
