...
test("css(String|Hash)", function() {
	...
	equal( jQuery("#qunit-fixture").css("display"), "block", "Check for styles property \"display\"" );
	...
	equal( typeof child.css("width"), "string", "Make sure that a string width is returned from styles('width')." );
...
test("css() explicit and relative values", function() {
	...
	equal( $elem.css("width"), "1px", "Initial styles set or width/height works (hash)" );
	equal( $elem.css("paddingLeft"), "1px", "Initial styles set of paddingLeft works (hash)" );
	equal( $elem.css("opacity"), "1", "Initial styles set of opacity works (hash)" );
...
test("css(String, Object)", function() {
	...
	equal( j.css("overflow"), "visible", "Check node,textnode,comment styles works" );
...
test("jQuery.styles(elem, 'height') doesn't clear radio buttons (bug #1095)", function () {
	expect(4);
...
test("box model properties incorrectly returning % instead of px, see #10639 and #12088", function() {
	...
	equal( el.css("marginRight"), "200px", "styles('marginRight') returning % instead of px, see #10639" );
	equal( el2.css("marginLeft"), "100px", "styles('marginLeft') returning incorrect pixel value, see #12088" );
});
...
test("can't get styles for disconnected in IE<9, see #10254 and #8388", function() {
	expect( 2 );
	var span, div;
...
test("styles('width') and styles('height') should respect box-sizing, see #11004", function() {
	expect( 4 );
	...
	equal( el.css("width"), el.css("width", el.css("width")).css("width"), "styles('width') is not respecting box-sizing, see #11004");
	equal( el_dis.css("width"), el_dis.css("width", el_dis.css("width")).css("width"), "styles('width') is not respecting box-sizing for disconnected element, see #11004");
	equal( el.css("height"), el.css("height", el.css("height")).css("height"), "styles('height') is not respecting box-sizing, see #11004");
	equal( el_dis.css("height"), el_dis.css("height", el_dis.css("height")).css("height"), "styles('height') is not respecting box-sizing for disconnected element, see #11004");
});

test("certain styles values of 'normal' should be convertable to a number, see #8627", function() {
	expect ( 2 );

	var el = jQuery("<div style='letter-spacing:normal;font-weight:normal;'>test</div>").appendTo("#qunit-fixture");

	ok( jQuery.isNumeric( parseFloat( el.css("letterSpacing") ) ), "styles('letterSpacing') not convertable to number, see #8627" );
	ok( jQuery.isNumeric( parseFloat( el.css("fontWeight") ) ), "styles('fontWeight') not convertable to number, see #8627" );
});

// only run this test in IE9
if ( document.documentMode === 9 ) {
	test( ".styles('filter') returns a string in IE9, see #12537", 1, function() {
		equal( jQuery("<div style='-ms-filter:\"progid:DXImageTransform.Microsoft.gradient(startColorstr=#FFFFFF, endColorstr=#ECECEC)\";'></div>").css("filter"), "progid:DXImageTransform.Microsoft.gradient(startColorstr=#FFFFFF, endColorstr=#ECECEC)", "IE9 returns the correct value from styles('filter')." );
	});
}
...
test( "styles opacity consistency across browsers (#12685)", function() {
	expect( 4 );
...
asyncTest( "Clearing a Cloned Element's Style Shouldn't Clear the Original Element's Style (#8908)", 24, function() {
	...
	jQuery.each(styles, function( index, style ) {
		...
		window.setTimeout(function() {
			notEqual( $clone.css( style.name ), style.value[ 0 ], "Cloned styles was changed" );

			ok( jQuery.inArray( $source.css( style.name ) !== -1, style.value ),
				"Clearing clone.styles() doesn't affect source.styles(): " + style.name +
				"; result: " + $source.css( style.name ) +
				"; expected: " + style.value.join( "," ) );

			ok( jQuery.inArray( $children.css( style.name ) !== -1, style.value ),
				"Clearing clonedChildren.styles() doesn't affect children.styles(): " + style.name +
